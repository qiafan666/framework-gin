...
func (ws *WsServer) ChangeOnlineStatus(concurrent int) {
	...
	for {
		...
		case now := <-renewalTicker.C: // 每次 renewalTicker 触发时，检查需要更新的用户状态
			...
			glog.Slog.DebugKVs(function.WsCtx, "CheckOnlineStatus renewal ticker", "deadline", deadline.Format(time.DateTime), "nowtime", now.Format(time.DateTime), "num", len(users), "users", users)
