...
func NewUserOnline(rdb redis.UniversalClient) OnlineCache {
	return &userOnline{
		rdb:         rdb,
		expire:      constant.OnlineExpire,
		channelName: constant.OnlineChannel,
	}
}
...
func (s *userOnline) getUserOnlineKey(userID string) string {
	return constant.GetOnlineKey(userID)
}
...
`
	...
	if err != nil {
		glog.Slog.ErrorKVs(ctx, "redis SetUserOnline", err, "userID", userID, "online", online, "offline", offline)
		return err
	}
	glog.Slog.DebugKVs(ctx, "redis SetUserOnline", "userID", userID, "online", online, "offline", offline, "status", status)
	return nil
}
